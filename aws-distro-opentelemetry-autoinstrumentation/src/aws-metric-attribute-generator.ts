...
import { AttributeMap, DEPENDENCY_METRIC, MetricAttributeGenerator, SERVICE_METRIC } from './metric-attribute-generator';
...
export class AwsMetricAttributeGenerator implements MetricAttributeGenerator {
  ...
  private static setRemoteServiceAndOperation(span: ReadableSpan, builder: Attributes): void {
    ...
    if (AwsSpanProcessingUtil.isKeyPresent(span, AWS_ATTRIBUTE_KEYS.AWS_REMOTE_SERVICE) || AwsSpanProcessingUtil.isKeyPresent(span, AWS_ATTRIBUTE_KEYS.AWS_REMOTE_OPERATION)) {
    ...
    } else if (AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_RPC_SERVICE) || AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_RPC_METHOD)) {
      remoteService = AwsMetricAttributeGenerator.normalizeRemoteServiceName(span, AwsMetricAttributeGenerator.getRemoteService(span, SEMATTRS_RPC_SERVICE));
    ...
    } else if (AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_FAAS_INVOKED_NAME) || AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_FAAS_TRIGGER)) {
    ...
    } else if (AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_MESSAGING_SYSTEM) || AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_MESSAGING_OPERATION)) {
    ...
    if (AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_PEER_SERVICE) && !AwsSpanProcessingUtil.isKeyPresent(span, AWS_ATTRIBUTE_KEYS.AWS_REMOTE_SERVICE)) {
  ...
  private static setRemoteResourceTypeAndIdentifier(span: ReadableSpan, builder: Attributes): void {
    ...
    if (AwsSpanProcessingUtil.isAwsSDKSpan(span)) {
      if (AwsSpanProcessingUtil.isKeyPresent(span, AWS_ATTRIBUTE_KEYS.AWS_TABLE_NAME)) {
        ...
        remoteResourceIdentifier = AwsMetricAttributeGenerator.escapeDelimiters(span.attributes[AWS_ATTRIBUTE_KEYS.AWS_TABLE_NAME]);
        ...
        remoteResourceIdentifier = AwsMetricAttributeGenerator.escapeDelimiters(span.attributes[AWS_ATTRIBUTE_KEYS.AWS_STREAM_NAME]);
        ...
        remoteResourceIdentifier = AwsMetricAttributeGenerator.escapeDelimiters(span.attributes[AWS_ATTRIBUTE_KEYS.AWS_BUCKET_NAME]);
        ...
        remoteResourceIdentifier = AwsMetricAttributeGenerator.escapeDelimiters(span.attributes[AWS_ATTRIBUTE_KEYS.AWS_QUEUE_NAME]);
      } else if (AwsSpanProcessingUtil.isKeyPresent(span, AWS_ATTRIBUTE_KEYS.AWS_QUEUE_URL)) {
        remoteResourceType = NORMALIZED_SQS_SERVICE_NAME + '::Queue';
        remoteResourceIdentifier = SqsUrlParser.getQueueName(AwsMetricAttributeGenerator.escapeDelimiters(span.attributes[AWS_ATTRIBUTE_KEYS.AWS_QUEUE_URL]));
      }
    } else if (AwsSpanProcessingUtil.isDBSpan(span)) {
  ...
  private static buildDbConnection(address: AttributeValue | undefined, port: AttributeValue | undefined): string | undefined {
  ...
  private static getDBStatementRemoteOperation(span: ReadableSpan, remoteOperationKey: string): string {
    ...
    const matcher: RegExpMatchArray | null = remoteOperation.toUpperCase().match(AwsSpanProcessingUtil.SQL_DIALECT_PATTERN);
