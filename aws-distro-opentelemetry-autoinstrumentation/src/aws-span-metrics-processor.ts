...
export class AwsSpanMetricsProcessor implements SpanProcessor {
  ...
  static create(errorHistogram: Histogram, faultHistogram: Histogram, latencyHistogram: Histogram, generator: MetricAttributeGenerator, resource: Resource): AwsSpanMetricsProcessor {
    return new AwsSpanMetricsProcessor(errorHistogram, faultHistogram, latencyHistogram, generator, resource);
  }

  private constructor(errorHistogram: Histogram, faultHistogram: Histogram, latencyHistogram: Histogram, generator: MetricAttributeGenerator, resource: Resource) {
  ...
  private recordErrorOrFault(spanData: ReadableSpan, attributes: Attributes): void {
    ...
    if (httpStatusCode == undefined || (httpStatusCode as number) < this.ERROR_CODE_LOWER_BOUND || (httpStatusCode as number) > this.FAULT_CODE_UPPER_BOUND) {
    ...
    } else if ((httpStatusCode as number) >= this.ERROR_CODE_LOWER_BOUND && (httpStatusCode as number) <= this.ERROR_CODE_UPPER_BOUND) {
      this.errorHistogram.record(1, attributes);
      this.faultHistogram.record(0, attributes);
    } else if ((httpStatusCode as number) >= this.FAULT_CODE_LOWER_BOUND && (httpStatusCode as number) <= this.FAULT_CODE_UPPER_BOUND) {
      this.errorHistogram.record(0, attributes);
      this.faultHistogram.record(1, attributes);
    }
  }
