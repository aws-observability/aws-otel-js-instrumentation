...
export class AwsSpanProcessingUtil {
  ...
  static isAwsSDKSpan(span: ReadableSpan): boolean {
    ...
    return 'aws-api' == rpcSystem.toString();
  }

  static shouldGenerateServiceMetricAttributes(span: ReadableSpan): boolean {
    return (AwsSpanProcessingUtil.isLocalRoot(span) && !AwsSpanProcessingUtil.isSqsReceiveMessageConsumerSpan(span)) || SpanKind.SERVER == span.kind;
  }
  ...
  static shouldGenerateDependencyMetricAttributes(span: ReadableSpan): boolean {
    ...
    if (isAwsSdkDescendent != undefined && (isAwsSdkDescendent as string) == 'true') {
      return false;
    }

    return SpanKind.CLIENT === span.kind || SpanKind.PRODUCER === span.kind || (AwsSpanProcessingUtil.isDependencyConsumerSpan(span) && !AwsSpanProcessingUtil.isSqsReceiveMessageConsumerSpan(span));
  }
  ...
  static isConsumerProcessSpan(spanData: ReadableSpan): boolean {
    ...
    return SpanKind.CONSUMER === spanData.kind && MessagingOperationValues.PROCESS == messagingOperation.toString();
  }
  ...
  private static isSqsReceiveMessageConsumerSpan(spanData: ReadableSpan): boolean {
    ...
    return (
      AwsSpanProcessingUtil.SQS_RECEIVE_MESSAGE_SPAN_NAME.toLocaleLowerCase() == spanName.toLocaleLowerCase() &&
  ...
  private static isDependencyConsumerSpan(span: ReadableSpan): boolean {
    if (SpanKind.CONSUMER !== span.kind) {
      ...
      const parentSpanKind: AttributeValue | undefined = span.attributes[AWS_ATTRIBUTE_KEYS.AWS_CONSUMER_PARENT_SPAN_KIND];

      return SpanKind[SpanKind.CONSUMER] != (parentSpanKind as string);
    }
    return true;
  }
  ...
  private static isValidOperation(span: ReadableSpan, operation: string): boolean {
    ...
    if (AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_HTTP_METHOD)) {
      ...
      // `httpMethod!` tells TypeScript that `httpMethod` will not be undefined here
      // We know this is not undefined because the key is present
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      return operation != httpMethod!.toString();
    }
    return true;
  }
  ...
  static isDBSpan(span: ReadableSpan): boolean {
    return (
      AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_DB_SYSTEM) || AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_DB_OPERATION) || AwsSpanProcessingUtil.isKeyPresent(span, SEMATTRS_DB_STATEMENT)
    );
  }
