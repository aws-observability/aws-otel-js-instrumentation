...
export abstract class EMFExporterBase implements PushMetricExporter {
   ...
   * @returns {Promise<void>}
   */
  protected abstract sendLogEvent(logEvent: Required<LogEvent>): Promise<void>;

  /**
   * Force flush any pending metrics.
   * This method must be implemented by subclasses to forceFlush pending metrics.
   */
  abstract forceFlush(): Promise<void>;

  /**
   * Shutdown the exporter.
   * This method must be implemented by subclasses to gracefully shutdown
   * and clean-up the exporter.
   */
