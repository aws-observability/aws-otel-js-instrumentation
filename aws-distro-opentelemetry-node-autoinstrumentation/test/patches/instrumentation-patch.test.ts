  ...
  describe('AwsSdkInstrumentationPatchTest', () => {
    ...
    it('injects trace context header into request via propagator', async () => {
      ...
      expect(reqHeaders['x-amzn-trace-id'] as string).toEqual(
        `Root=1-${listObjectsSpans[0].spanContext().traceId.substring(0, 8)}-${listObjectsSpans[0]
          .spanContext()
          .traceId.substring(8, 32)};Parent=${listObjectsSpans[0].spanContext().spanId};Sampled=1`
      );
    });
  });
});
