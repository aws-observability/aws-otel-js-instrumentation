...
describe('AwsXrayRemoteSampler', () => {
  it('testCreateRemoteSamplerWithEmptyResource', () => {
    var sampler: AwsXRayRemoteSampler = new AwsXRayRemoteSampler({resource: Resource.EMPTY});
  ...
  it('testUpdateSamplingRulesAndTargetsWithPollersAndShouldSampled', (done) => {
    ...
    let tmp = (AwsXRayRemoteSampler.prototype as any).getDefaultTargetPollingInterval;
    ...
    setTimeout(() => {
      ...
      expect(sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, {"abc": "1234"}, []).decision).toEqual(SamplingDecision.NOT_RECORD);

      setTimeout(() => {
        expect(sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, {"abc": "1234"}, []).decision).toEqual(SamplingDecision.RECORD_AND_SAMPLED);
        expect(sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, {"abc": "1234"}, []).decision).toEqual(SamplingDecision.RECORD_AND_SAMPLED);
        expect(sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, {"abc": "1234"}, []).decision).toEqual(SamplingDecision.RECORD_AND_SAMPLED);
  ...
  it('testLargeReservoir', (done) => {
    ...
    setTimeout(() => {
      expect(sampler.getRuleCache().getRuleAppliers()[0].samplingRule.RuleName).toEqual('test');
      expect(sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, attributes, []).decision).toEqual(SamplingDecision.NOT_RECORD);

      setTimeout(() => {
        let sampled = 0;
        for (let i = 0; i < 100000; i++) {
          if (sampler.shouldSample(context.active(), '1234', "name", SpanKind.CLIENT, attributes, []).decision != SamplingDecision.NOT_RECORD) {
            sampled++;
          }
        }
