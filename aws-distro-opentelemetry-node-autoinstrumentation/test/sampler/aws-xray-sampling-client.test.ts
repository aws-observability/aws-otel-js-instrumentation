...
import { GetSamplingRulesResponse, GetSamplingTargetsBody, GetSamplingTargetsResponse } from '../../src/sampler/remote-sampler.types';
...
describe('AwsXraySamplingClient', () => {
  ...
  it('testDefaultValuesUsedWhenMissingPropertiesInSamplingRule', (done) => {
    nock(TEST_URL).post('/GetSamplingRules').reply(200, {"SamplingRuleRecords": [{"SamplingRule": {}}]});
    const client = new AwsXraySamplingClient(TEST_URL, new DiagConsoleLogger());
    client.fetchSamplingRules((response:GetSamplingRulesResponse) => {
      expect(response.SamplingRuleRecords?.length).toEqual(1);
  ...
  it('testGetCorrectNumberOfSamplingRules', (done) => {
    ...
    client.fetchSamplingRules((response:GetSamplingRulesResponse) => {
      ...
      for (let i = 0; i < records.length; i ++) {
        ...
        expect(response.SamplingRuleRecords?.[i].SamplingRule?.ReservoirSize).toEqual(records[i].SamplingRule.ReservoirSize);
        expect(response.SamplingRuleRecords?.[i].SamplingRule?.ResourceARN).toEqual(records[i].SamplingRule.ResourceARN);
        ...
        expect(response.SamplingRuleRecords?.[i].SamplingRule?.ServiceName).toEqual(records[i].SamplingRule.ServiceName);
        expect(response.SamplingRuleRecords?.[i].SamplingRule?.ServiceType).toEqual(records[i].SamplingRule.ServiceType);
