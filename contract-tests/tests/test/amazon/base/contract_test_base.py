...
def can_convert_any_value_to_string(any_value_instance):
    field_name = any_value_instance.WhichOneof('value')

    if field_name in ['string_value', 'bool_value', 'int_value', 'double_value']:
        return True

    elif field_name == 'bytes_value':
        # Check if bytes can be decoded to a string
        try:
            any_value_instance.bytes_value.decode('utf-8')
            return True
        ...
        return all(can_convert_any_value_to_string(item) for item in any_value_instance.array_value.values)

    elif field_name == 'kvlist_value':
        # Check if all values can be converted
        return all(can_convert_any_value_to_string(kv.value) for kv in any_value_instance.kvlist_value.values)

    else:
        # Unknown field; cannot determine
        return False
...
class ContractTestBase(TestCase):
    ...
    def _get_attributes_dict(self, attributes_list: List[KeyValue]) -> Dict[str, AnyValue]:
        ...
        for attribute in attributes_list:
            ...
            # if can_convert_any_value_to_string(value):
