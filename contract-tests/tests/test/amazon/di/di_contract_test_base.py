...
Follows the same mock-collector pattern as ServiceEventsTestInfrastructure:
snapshots are emitted as OTLP LogRecords to the mock collector via HTTP,
and tests query the collector via MockCollectorClient (gRPC).

The mock collector exposes:
  - gRPC on port 4315 (used by Application Signals / ServiceEvents / test client)
  - HTTP on port 4318 (used by DI snapshot OTLP HTTP exporter)
...
# ---------------------------------------------------------------------------
# Protobuf conversion helpers (same as serviceevents_contract_test_base.py)
# ---------------------------------------------------------------------------
