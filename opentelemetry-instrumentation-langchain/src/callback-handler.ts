...
export class OTelCallbackHandler extends BaseCallbackHandler {
  ...
  /**
   * Extracts LangGraph-specific attributes from metadata.
   * @param metadata - Callback metadata that may contain LangGraph info.
   * @returns Object with langgraph_node and langgraph_step if present.
   */
  private extractLangGraphAttributes(metadata?: Record<string, unknown>): Record<string, string | number> {
    const attrs: Record<string, string | number> = {};
    if (!metadata) return attrs;

    if (typeof metadata.langgraph_node === 'string') {
      attrs[ATTR_GEN_AI_LANGGRAPH_NODE] = metadata.langgraph_node;
    }
    if (typeof metadata.langgraph_step === 'number') {
      attrs[ATTR_GEN_AI_LANGGRAPH_STEP] = metadata.langgraph_step;
    }
    return attrs;
  }
  ...
  private startSpan(
    ...
    // Extract and set LangGraph-specific attributes
    const langGraphAttrs = this.extractLangGraphAttributes(metadata);
    for (const [key, value] of Object.entries(langGraphAttrs)) {
      span.setAttribute(key, value);
    }
  ...
  override handleChatModelStart(
    llm: { id?: string[]; kwargs?: { model?: string; model_id?: string }; lc_kwargs?: { model?: string; model_id?: string } },
    messages: unknown[][],
    runId: string,
    parentRunId?: string,
    ...
    // Model is not directly available - need to get from metadata or use placeholder
    // The actual model will be set in handleLLMEnd from response_metadata
    const model = (metadata?.model as string) || (metadata?.model_id as string) || 'pending';
