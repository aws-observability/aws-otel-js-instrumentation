...
import {
  ...
  ATTR_GEN_AI_LANGGRAPH_NODE,
  ATTR_GEN_AI_LANGGRAPH_STEP,
} from '../src/types';
...
describe('OTelCallbackHandler', () => {
  ...
  describe('handleChatModelStart', () => {
    it('creates span with correct attributes and SpanKind.CLIENT', () => {
      ...
      expect(span.attributes[ATTR_GEN_AI_REQUEST_MODEL]).toBe('claude-3');
    ...
    it('extracts openai provider correctly', () => {
      ...
      handler.handleChatModelStart(llm, [[]], runId, undefined, { model: 'gpt-4' });
      handler.handleLLMEnd({ llmOutput: {} }, runId);
    ...
    it('extracts anthropic provider correctly', () => {
      ...
      handler.handleChatModelStart(llm, [[]], runId, undefined, { model: 'claude-2' });
      handler.handleLLMEnd({ llmOutput: {} }, runId);
  ...
  describe('handleToolStart', () => {
    it('creates child span with ATTR_GEN_AI_TOOL_NAME', () => {
      ...
      handler.handleChatModelStart(llm, [[]], parentRunId, undefined, { model: 'gpt-4' });
  ...
  describe('parent-child relationships', () => {
    it('maintains correct span hierarchy via runId mapping', () => {
      ...
      handler.handleChatModelStart(llm, [[]], llmRunId, chainRunId, {}, [], { model: 'gpt-4' });
      ...
      const llmSpan = spans.find(s => s.name.includes('gpt-4'));
  ...
  describe('LangGraph attributes', () => {
    it('extracts langgraph_node and langgraph_step from metadata', () => {
      ...
      const spans = exporter.getFinishedSpans();
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_NODE]).toBe('agent');
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_STEP]).toBe(1);
    });
    ...
    it('handles missing langgraph metadata gracefully', () => {
      ...
      const spans = exporter.getFinishedSpans();
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_NODE]).toBeUndefined();
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_STEP]).toBeUndefined();
    });

    it('sets langgraph attributes on chain spans', () => {
      ...
      const spans = exporter.getFinishedSpans();
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_NODE]).toBe('tools');
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_STEP]).toBe(2);
    });

    it('sets langgraph attributes on tool spans', () => {
      ...
      const spans = exporter.getFinishedSpans();
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_NODE]).toBe('tools');
      expect(spans[0].attributes[ATTR_GEN_AI_LANGGRAPH_STEP]).toBe(3);
    });
  });
  ...
  describe('handleLLMStart', () => {
    it('normalizes prompts to OTel format with parts', () => {
      ...
      const llm = { id: ['langchain', 'llms', 'openai'], kwargs: { model: 'gpt-3.5' } };
